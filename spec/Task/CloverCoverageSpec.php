...
class CloverCoverageSpec extends ObjectBehavior
{
    ...
    public function it_should_not_run_in_commit_message_context(GitCommitMsgContext $context)
    {
        $this->canRunInContext($context)->shouldReturn(false);
    }

    public function it_runs_the_suite_but_fails_when_file_doesnt_exists(GrumPHP $grumPHP, GitCommitMsgContext $context)
    {
        $grumPHP->getTaskConfiguration('clover_coverage')->willReturn([
            'clover_file' => 'foo.bar',
        ]);
    ...
    public function it_runs_the_suite_but_not_reaching_coverage(GrumPHP $grumPHP, GitCommitMsgContext $context)
    {
        $filename = dirname(dirname(__DIR__)) . '/test/fixtures/clover_coverage/60-percent-coverage.xml';
        $grumPHP->getTaskConfiguration('clover_coverage')->willReturn([
            'clover_file' => $filename,
