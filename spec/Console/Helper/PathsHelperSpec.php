...
{
    public function let(GrumPHP $config, Filesystem $filesystem, ExternalCommand $externalCommandLocator)
    {
    ...
    public function it_throws_exception_during_get_relative_path_when_file_is_not_found()
    {
        $path = '/non/existent/path/config.yml';
        $this->shouldThrow(FileNotFoundException::class)->duringGetRelativePath($path);
    }

    public function it_knows_the_default_configuration_file()
    {
        $this->getDefaultConfigPath()->shouldBe('/grumphp.yml');
    }

    public function it_does_not_load_ascii_art_when_null_is_configured(GrumPHP $config)
    {
        $config->getAsciiContentPath('resource')->willReturn(null);
        $this->getAsciiContent('resource')->shouldBe('');
    }

    public function it_can_load_ascii_art_from_a_user_defined_resource(GrumPHP $config, Filesystem $filesystem)
    {
    ...
    public function it_can_load_embedded_ascii_art_when_the_user_defned_resource_does_not_exist(GrumPHP $config, Filesystem $filesystem)
    {
    ...
    public function it_embeds_an_error_message_if_the_ascii_art_could_not_be_found(GrumPHP $config, Filesystem $filesystem)
    {
