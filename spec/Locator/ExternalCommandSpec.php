...
{
    public function let(ExecutableFinder $executableFinder)
    {
    ...
    public function it_throws_exception_when_external_command_is_not_found(ExecutableFinder $executableFinder)
    {
        $executableFinder->find('test', null, ['bin'])->willReturn(false);
        $this->shouldThrow(RuntimeException::class)->duringLocate('test');
    }

    public function it_locates_external_commands(ExecutableFinder $executableFinder)
    {
